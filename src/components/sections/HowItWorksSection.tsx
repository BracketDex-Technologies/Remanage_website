...
export function HowItWorksSection() {
  return (
    <section id="platform" className="relative overflow-hidden py-10 md:py-12">
      <div className="absolute inset-0 z-0 bg-slate-950">
        ...
        <div
          className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-br from-slate-950 from-0% via-slate-900/95 via-45% to-blue-900/70 to-100%"
          aria-hidden
        />
      </div>

      <div className="pointer-events-none relative z-10 mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-10 xl:px-12">
        <SectionHeading
        ...
        <div className="pointer-events-none grid gap-px border border-white/20 bg-white/5 sm:grid-cols-3">
          {howItWorks.map((item, index) => (
            <motion.div
              ...
              className="bg-slate-900/25 p-5 backdrop-blur-[2px] sm:p-6"
            >
              <span className="font-heading text-3xl font-semibold text-sky-300/50">{item.step}</span>
        ...
        <div className="pointer-events-none flex flex-col gap-5 border-t border-white/15 pt-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="grid flex-1 grid-cols-3 gap-4">
            {stats.map((stat) => (
              <div key={stat.label} className="border-l border-sky-400/40 pl-4">
